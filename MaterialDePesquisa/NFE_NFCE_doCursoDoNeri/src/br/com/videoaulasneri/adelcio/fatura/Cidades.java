...
Descrição: Manutenção da Tabela de Cidades
...
public class Cidades extends JFrame implements ActionListener, MouseListener, KeyListener, FocusListener
{
    int navega = 0;
    ...
    String acaoFiltro;
    JLabel lb_titulo = new JLabel("Manutencao de Cadastro: Cidades");

    //Labels dos campos da tabela;
    JLabel label_codigo              = new JLabel("Codigo: ");
    JLabel label_cidade              = new JLabel("Cidade: ");
    JLabel label_codcidade           = new JLabel("Codcidade: ");
    JLabel label_distrito            = new JLabel("Distrito: ");
    ...
    JTextField tf_distrito           = new JTextField();
    JTextField tf_uf                 = new JTextField();
    ...
    int posUltLabel                  = 0;
    int qtRegTab                     = 0;
    JTextField tf_aux_pesq           = tf_cidade;
    PreparedStatement ps_aux;
    int empresa = 0;
    String razaoEmp, enderEmp;

    public Cidades(int empresa, Connection confat, String razaoEmpresa, String enderEmpresa)
    {
        setTitle("Formulario de Manutencao de Cidades");
        setSize(1010, 680);
        setLocation(135,85);
        setResizable(true);
    ...
    //Gera cor frente dos lanbels setForegroundColor()
        ...
        label_cidade            .setForeground(Color.black);
        label_codcidade         .setForeground(Color.black);
        label_distrito          .setForeground(Color.black);
        label_uf                .setForeground(Color.black);


    //alinha os labels a direita
        label_codigo            .setHorizontalAlignment(JLabel.RIGHT);
        label_cidade            .setHorizontalAlignment(JLabel.RIGHT);
        label_codcidade         .setHorizontalAlignment(JLabel.RIGHT);
        label_distrito          .setHorizontalAlignment(JLabel.RIGHT);
        label_uf                .setHorizontalAlignment(JLabel.RIGHT);
    ...
    //Gera cor frente dos texfields setForegroundColor()
        ...
        tf_distrito             .setForeground(Color.black);
    ...
    //Gera cor frente de fundo texfields setBackroundColor()
        ...
        tf_distrito             .setBackground(Color.white);
        ...
        label_cidade             .setBounds(100,190,90,20);
        tf_cidade                .setBounds(190,190,300,20);
        label_codcidade          .setBounds(100,215,90,20);
        tf_codcidade             .setBounds(190,215,88,20);
        label_distrito           .setBounds(100,240,90,20);
        tf_distrito              .setBounds(190,240,300,20);
        label_uf                 .setBounds(100,265,90,20);
        tf_uf                    .setBounds(190,265,30,20);
//Posicao do ultimo cpo da Tabela no Formulario. Utiliz.para Iniciar a exibicao do JTable
        ...
        jTable1.setDefaultRenderer(Object.class, new CellRenderer_ibge());
        ...
        //getContentPane()      .add(label_codpalmtop);
        //getContentPane()      .add(tf_codpalmtop);
        //getContentPane()      .add(label_codsicredi);
        //getContentPane()      .add(tf_codsicredi);
        //getContentPane()      .add(label_codsif);
        //getContentPane()      .add(tf_codsif);
        getContentPane()      .add(label_distrito);
        getContentPane()      .add(tf_distrito);
        getContentPane()      .add(label_uf);
        ...
        cpo_tabela[0]         = "codigo";
        cpo_tabela[1]         = "cidade";
        cpo_tabela[2]         = "codcidade";
        cpo_tabela[3]         = "distrito";
        cpo_tabela[4]         = "uf";
        lab_tabela[0]         = "Codigo";
        lab_tabela[1]         = "Cidade";
        lab_tabela[2]         = "Codcidade";
        lab_tabela[3]         = "Distrito";
        lab_tabela[4]         = "Uf";
    ...
    public void preencher_cb_pesquisa(String acaoFiltro){
         ...
         try {
             ...
             if (acaoFiltro.equals("filtrar") && !tf_pesquisa.getText().equals("")){
                if (cb_pesq_lab.getSelectedIndex() == 1 || cb_pesq_lab.getSelectedIndex() == 3) {
                  sql_query = "select * from ibge where "
             ...
             while(resultset.next()) {
                 ...
                 //cb_pesquisa.addItem(resultset.getString("cidade"));
             }
             resultset.last();
    ...
    public void preencher_jtable(String acaoFiltro)
    {
        ...
        this.acaoFiltro = acaoFiltro;
        //try {
        //    resultset = statement.executeQuery("select * from ibge");
        //} catch (SQLException ex) {
        //    JOptionPane.showMessageDialog(null,"Erro ao listar ibge: "+ex);
        //}
        ...
        try
        {
            ...
            if (acaoFiltro.equals("filtrar") && !tf_pesquisa.getText().equals("")){
                if (cb_pesq_lab.getSelectedIndex() == 1 || cb_pesq_lab.getSelectedIndex() == 3) {
                  sqlquery = "select * from ibge where "
                                 +cpo_tabela[cb_pesq_lab.getSelectedIndex()]+" like '%"+tf_pesquisa.getText()+"%'";
                } else {
                  sqlquery = "select * from ibge where "
                                 +cpo_tabela[cb_pesq_lab.getSelectedIndex()]+" = "+tf_pesquisa.getText()+"";
            ...
            //JOptionPane.showMessageDialog(null,"Qtde regs da tabela: ibge: "+qtreg);
            resultset.last();
        }
   ...
   public void lookandfeel()
   {
    ...
    public void mostra_conteudo_nos_campos()
    {
           ...
           try
           {
               ...
               tf_cidade             .setText(resultset.getString("cidade"));
               tf_codcidade          .setText(resultset.getString("codcidade"));
               //tf_codpalmtop         .setText(resultset.getString("codpalmtop"));
               //tf_codsicredi         .setText(resultset.getString("codsicredi"));
               //tf_codsif             .setText(resultset.getString("codsif"));
               tf_distrito           .setText(resultset.getString("distrito"));
               tf_uf                 .setText(resultset.getString("uf"));
...
//Desabilita campo chave para alteracao/exclusao 
    ...
    public void novo_registro()
    {
        ...
        //tf_codpalmtop         .setText("");
        //tf_codsicredi         .setText("");
        //tf_codsif             .setText("");
        tf_distrito           .setText("");
        tf_uf                 .setText("");
      posiciona_combo_e_jtable();
...
//Habilita campo chave para inclusao 
    ...
    public boolean check_textField() {
        ...
        if ( tf_cidade.getText() == null ) tf_cidade.setText("");
        if ( tf_codcidade.getText() == null ) tf_codcidade.setText("");
        //if ( tf_codpalmtop.getText() == null ) tf_codpalmtop.setText("");
        //if ( tf_codsicredi.getText() == null ) tf_codsicredi.setText("");
        //if ( tf_codsif.getText() == null ) tf_codsif.setText("");
        if ( tf_distrito.getText() == null ) tf_distrito.setText("");
        if ( tf_uf.getText() == null ) tf_uf.setText("");
        tf_cidade.setText(tf_cidade.getText().toUpperCase());
        tf_distrito.setText(tf_distrito.getText().toUpperCase());
        return retorno;
    }
    ...
    public void gravar()
    {
        if (ehnovo)
            ...
            try
            {
                 String sql_insert = "insert into ibge ( "+
                        "codigo, "+"cidade, "+"codcidade, "+
                        "distrito, "+"uf "+")"+
                        " values ("+tf_codigo.getText() + ", "+"'"+tf_cidade.getText() + "', "+tf_codcidade.getText() + ", "+
                        "'"+tf_distrito.getText() + "', "+
                        "'"+tf_uf.getText() + "' " + ")";
    ...
    public void alterar()
    {
        if (!ehnovo)
        {
             try
            {
                 String sql_alterar = "update ibge set " +
                         "cidade = '"+ tf_cidade.getText()+"', "+
                         "codcidade = "+ tf_codcidade.getText()+", "+
                         "distrito = '"+ tf_distrito.getText()+"', "+
                         "uf = '"+ tf_uf.getText()+"'" +
     ...
     public void excluir()
     {
       try
       {
           String nome = "Deletar ibge : "+tf_cidade.getText()+" ?";
           System.out.println("nome = " + nome);
           int opcao_escolhida = JOptionPane.showConfirmDialog(null,nome,"Exclusao ",JOptionPane.YES_NO_OPTION);
           if (opcao_escolhida == JOptionPane.YES_OPTION)
           {
               String  sql_delete = "DELETE FROM ibge Where codigo = " + tf_codigo.getText();
    ...
    public void imprimir()
    {
        ...
        Impressao impr = new Impressao(""+empresa, "ibge.jasper", connection, razaoEmp, enderEmp);
    ...
    public void pesquisadigitacao()
    {
       try
       {
           ...
           if (achou){
                ...
                cb_pesquisa.setSelectedItem(tf_cidade.getText());
     ...
     if ( cb_pesquisa.getSelectedItem() != null ){
       try
       {
           ...
           while(!achou)
           {
            ...
            clicado = true;
/*
        String sql = "select * from ibge where Descricao = "+cb_pesquisa.getSelectedItem();
...
*/
    ...
    public void trataEvento_mouse_key_jTable1(){
            ...
            try {
                if (acaoFiltro.equals("filtrar") && !tf_pesquisa.getText().equals("")){
                    String sql_query = "select * from ibge where "
                                +cpo_tabela[cb_pesq_lab.getSelectedIndex()]+" like '%"+tf_pesquisa.getText()+"%'";
    ...
    public static void main(String args[])
        {
        JFrame form = new Cidades(1, null, null, null);
            form.setVisible(true);
    }
}
////classe auxiliar para centralizar as colunas dp JTable
class CellRenderer_ibge extends DefaultTableCellRenderer {
