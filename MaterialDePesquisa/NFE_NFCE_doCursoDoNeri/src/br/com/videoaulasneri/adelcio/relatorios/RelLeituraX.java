...
import br.com.videoaulasneri.adelcio.fatura.bean.LeituraX;
...
public class RelLeituraX extends JFrame implements ActionListener{
    ...
    int empresa;
    String sempresa = "";
    String nomeBanco, razaoEmp, enderEmp;
    Date dateIni, dateFim;
    public RelLeituraX(int empresa, Connection conex, String razaoEmpresa, String enderEmpresa){
        try {
            ...
            tf_dataini.setText(currDateString);
            tf_datafim.setText(currDateString);
        } catch(Exception e) {
            JOptionPane.showMessageDialog(null, "Relfatper() - erro: "+e);
        }
    }
    ...
    public void initComponents(){
        try {
            setTitle("Impressão da Leitura X");
            setSize(350, 240);
            ...
            JOptionPane.showMessageDialog(null, "LeituraX().initComponents() - erro: "+e);
        }
    }
      ...
      public boolean checkDatas(JTextField data1, JTextField data2){
            if (data1.getText().equals("") || data2.getText().equals("")) {
                ...
                try {
                    ...
                    if (dataTst2.before(dataTst1)) {
                        ...
                        return false;
                    } else if (dataTst1.after(new Date()) || dataTst2.after(new Date())) {
                        //JOptionPane.showMessageDialog(null, "Data Final de "+tipo+" não pode ser Menor que Data Inicial!");
                        txtErro = txtErro + "\n"+"Data Inicial/Final não pode ser Maior que data atual!";
                        return false;
                    } else
                        return true;
     ...
     public void imprime_fatcli(){
        try{
            ...
            String arquivo = report_dir+"LeituraX.jasper";  //+"movper.jasper";
//JOptionPane.showMessageDialog(null, "Arquivo Jasper: "+arquivo);
            ...
            if (retorno) {
                ...
                parameterMap.put("nome_empresa", razaoEmp);
...
//JOptionPane.showMessageDialog(null, "Executou ViewReport . . . ");
     ...
     private boolean gerarRelatorio() {
         boolean retorno = true;
         String sql;
         Statement stateNF, stateLX;
         ResultSet rs;
        try {
            stateLX = confat.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
//apagar reg antigos de LeituraX aqui   
            sql = "delete from leiturax";
            stateLX.execute(sql);
//ler regs da tabela nf e gravar na tabela leiturax            
            sql = "select * from nf where empresa = "+empresa+" and "
                    + "numero_nfe > 0 and modelonfe = '65' and "
                    + "data_emissao between '"+dateIni+"'  and '"+dateFim+"'";
            stateNF = confat.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
            rs = stateNF.executeQuery(sql);
            while (rs.next()) {
               LeituraX leituraX = new LeituraX();
               leituraX.setNumeroNFCe(rs.getInt("numero_nfe"));
               leituraX.setSerieNFCe(rs.getString("serie_nfe"));
               leituraX.setTipo("NFCe");
               leituraX.setData(rs.getDate("data_emissao"));
               leituraX.setValor(rs.getDouble("valor_total"));
               sql = "insert into leiturax ("
                       + "tipo, numeronfce, serienfce, data, valor"
                       + ") values ("
                       + "'" + leituraX.getTipo()+"', "
                       + leituraX.getNumeroNFCe()+", "
                       + "'"+leituraX.getSerieNFCe()+"', "
                       + "'"+leituraX.getData()+"', "
                       + leituraX.getValor()
                       +")";
               System.out.println("Comando sql nf x leiturax; "+sql);        
               stateLX.execute(sql);
            }
//ler regs da tabela suprimento e gravar na tabela leiturax            

            sql = "select * from suprimento where empresa = "+empresa+" and "
                    + "data_suprimento between '"+dateIni+"'  and '"+dateFim+"'";
            stateNF = confat.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
            rs = stateNF.executeQuery(sql);
            while (rs.next()) {
               LeituraX leituraX = new LeituraX();
               leituraX.setTipo("Suprimento");
               leituraX.setNumeroNFCe(0);
               leituraX.setSerieNFCe("");
               leituraX.setData(rs.getDate("data_suprimento"));
               leituraX.setValor(rs.getDouble("valor"));
               sql = "insert into leiturax ("
                       + "tipo, numeronfce, serienfce, data, valor"
                       + ") values ("
                       + "'" + leituraX.getTipo()+"', "
                       ...
                       + leituraX.getValor()
                       +")";
               System.out.println("Comando sql suprimento x leiturax; "+sql);        
               stateLX.execute(sql);
            }
//ler regs da tabela sangria e gravar na tabela leiturax            

            sql = "select * from sangria where empresa = "+empresa+" and "
                    + "data_sangria between '"+dateIni+"'  and '"+dateFim+"'";
            stateNF = confat.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
            rs = stateNF.executeQuery(sql);
            while (rs.next()) {
               LeituraX leituraX = new LeituraX();
               leituraX.setTipo("Sangria");
               leituraX.setNumeroNFCe(0);
               leituraX.setSerieNFCe("");
               leituraX.setData(rs.getDate("data_sangria"));
               leituraX.setValor(rs.getDouble("valor") * -1);
               sql = "insert into leiturax ("
                       + "tipo, numeronfce, serienfce, data, valor"
                       + ") values ("
                       + "'" + leituraX.getTipo()+"', "
                       ...
                       + leituraX.getValor()
                       +")";
               System.out.println("Comando sql sangria x leiturax; "+sql);        
               stateLX.execute(sql);
            }
        } catch (SQLException ex) {
            Logger.getLogger(RelLeituraX.class.getName()).log(Level.SEVERE, null, ex);
        }
         return retorno;
     }
    ...
    public static void main(String args[]) throws SQLException
        ...
        JFrame relfat = new RelLeituraX(2, null, "", "");
